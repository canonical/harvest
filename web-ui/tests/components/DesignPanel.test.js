...
const DEPLOYMENT_NO_DESIGN = {
  id: 'd1', design_doc: null,
};

const DEPLOYMENT_WITH_DESIGN = {
  id: 'd1', design_doc: { id: 'a1', title: 'Design' },
};

function mountPanel(deployment = DEPLOYMENT_NO_DESIGN) {
...
describe('DesignPanel', () => {
  ...
  it('shows generate button when no design doc exists', () => {
    api.getArtifact.mockResolvedValue({ content: '' });
    const w = mountPanel();
    expect(w.find('[data-testid="generate-design-btn"]').exists()).toBe(true);
  });

  it('shows rendered design and edit button when design doc exists', async () => {
    api.getArtifact.mockResolvedValue({ content: '# My Design' });
    const w = mountPanel(DEPLOYMENT_WITH_DESIGN);
    await flushPromises();
    expect(w.find('[data-testid="design-preview"]').exists()).toBe(true);
    expect(w.find('[data-testid="edit-design-btn"]').exists()).toBe(true);
  });

  it('generate calls generateDesign and emits refresh', async () => {
