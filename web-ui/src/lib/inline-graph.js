...
export function mountInlineGraphs(containerEl) {
  containerEl
    .querySelectorAll('.inline-graph[data-graph]:not(.inline-graph--mounted)')
    .forEach(mountOne);
}
...
function mountOne(el) {
  ...
  try {
    ...
    el.textContent = 'Could not parse graph.';
    return;
  }
  ...
  function tapNode(node) {
    ...
    node.select();
    window.dispatchEvent(new CustomEvent('open-source-panel', {
      detail: { nodeData: node.data(), repo, version, sections: buildSections(node) },
    }));
  }
  ...
  cy.on('tap', e => {
    if (e.target === cy) {
      ...
      window.dispatchEvent(new CustomEvent('close-source-panel'));
    }
  });
}
