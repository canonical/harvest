    ...
    for (const id of (byLevel.get(lvl) ?? [])) {
      ...
      node.innerHTML = `
        ...
        <div class="tg-node__actions" aria-hidden="true">
          ...
          <button class="tg-node__action-btn tg-node__action-btn--view"
            data-action="view" data-task-id="${esc(id)}" tabindex="-1"
...
export function attachGraphHandlers(container, { onSelect, onRun, onView, onEdit } = {}) {
  container.addEventListener('click', e => {
    ...
    if (actionBtn) {
      ...
      if (actionBtn.dataset.action === 'view') onView?.(id);
